...
map
ggsave("map_exports_africa_countries_scen2.pdf")
...
ggsave("stacked_bar_plot_scen10.pdf",width=6.8*0.9,height=2.8*0.9)
...
pf %>% group_by( destination_country ) %>% 
  ...
  arrange(desc(sum)) %>% slice(1:6) %>% pull(destination_country) -> top_6_c

pf %>% filter( year=="2020" | week=="52"  ) %>% 
  mutate( destination_country_new=ifelse(destination_country%in%top_6_c,as.character(destination_country),"other") ) %>% 
...
ggsave("line_plot_scen10.pdf",width=8*0.85,height=1.8*0.85)
...
pf_fly %>% filter( year=="2020" | week=="52"  ) %>% 
  ...
  theme( axis.title.x = element_blank(),
         ...
         #axis.text.x = element_blank(),
         axis.ticks.y = element_blank())
ggsave("line_plot_vol_scen10.pdf",width=7*0.85,height=1.5*0.85)
...
ggsave("frac_time_wuhan_all.pdf",width=4*0.85,height=2)
