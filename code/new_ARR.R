...
data_feb29=read.csv("table_getAreaStat_en_2020-02-29.csv")
...
data_mar19=read.csv("table_getAreaStat_en_2020-03-19.csv")
confirmed_deaths_nh<-data_mar19%>%
  group_by(provinceShortName)%>%
  filter(provinceShortName!="Hubei")%>%
  summarise(cumulative_deaths_province=sum(deadCount))%>%
  summarise(cumulative_deaths_total=sum(cumulative_deaths_province))

confirmed_deaths_h<-data_mar19%>%
  group_by(provinceShortName)%>%
  filter(provinceShortName=="Hubei")%>%
...
ARR=(confirmed_cases_nh$cumulative_cases_total/total_infections_nh)/(confirmed_cases_h$cumulative_cases_Hubei/total_infections_h)
