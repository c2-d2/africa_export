# read in confirmed case data from James' github
...
all_incidence_province<-confirmed_cases_date%>%
  ...
  mutate(n_onset_inflated=ifelse(dates%in%ascertainment_date_seq_1,
                                 ...
                                 ifelse(dates%in%ascertainment_date_seq_2,
                                        n_onset/ascertainment_rate_2,
                                        n_onset/ascertainment_rate_3)))%>%
  mutate(n_onset_inflated=ifelse(province_raw=="Hubei",n_onset,n_onset_inflated))%>%
...
columns<-c("dates","province_raw","n_smoothed","n_onset_inflated","n_infected")
...
## using ascertainment rates from Meier & Brockmann 
...
ascertainment_rate_beijing_1=10^2.5/(10^2.5+10^0.25)

ascertainment_rate_beijing_2=10^2.5/(10^2.5+10^0.25)
ascertainment_rate_exc_hubei_1=10^2/(10^2.5+10^2)
ascertainment_rate_exc_hubei_2=10^4/(10^4+10^2)

# estimate symptom onset and infection incidence by province
all_incidence_province_2<-confirmed_cases_date%>%
  group_by(province_raw)%>%
  mutate(n_smoothed=predict(gam(n~s(dates,spar=0.7),
                                family=poisson(link="log")),type="response",
                            newdata=dates))%>%
