...
plot_conf_onset <- function(all_incidence_province,confirmed_cases_date) {
                ...
                # all dates subset
                all_incidence_province_subset<-all_incidence_province %>% dplyr::select(any_of(columns) )
...
comp_cum_incidence <- function(all_incidence_province, filen) {
                ...
                dplyr::left_join( df,popn_size_provinces, by=c("province_raw"="province") ) -> df
...
comp_travel_rel_prev <- function(city_n_inf_caladj_den, rel_dur) {
                ...
                if(rel_dur == 2){
                                city_n_inf_caladj_den %>% filter( !is.na(n_infected_caladj) ) %>% 
                                                group_by( city ) %>% 
                                                dplyr::select( province,city,date,n_infected_caladj ) %>% 
                                                mutate( 
                                                ...
                                                dplyr::select( province,city,date,travel_prev ) %>% ungroup() -> df
                } else if (rel_dur == 5) {
                                city_n_inf_caladj_den %>% filter( !is.na(n_infected_caladj) ) %>% 
                                                group_by( city ) %>% 
                                                dplyr::select( province,city,date,n_infected_caladj ) %>% 
...
comp_travel_rel_prev_nonwuh_gap <- function(city_n_inf_caladj_den){
                ...
                non_wuh <- city_n_inf_caladj_den %>% filter( !is.na(n_infected_caladj) & city != "Wuhan") %>% 
                                ...
                                dplyr::select( province,city,date,travel_prev ) %>% ungroup()
...
adjust_prov_prev_by_city <- function(prov_inc_calibrated , prov_city_adjust, assignment="by_city"){
                ...
                table_key <- prov_city_adjust %>% dplyr::select(province,city) %>% 
                                tidyr::expand_grid(date=date_v)
                ...
                if(assignment=="by_city"){
                  ...
                  table_key %>% dplyr::left_join( prov_city_adjust, by=c("province","city") ) %>% 
                                  dplyr::left_join( prov_inc_calibrated, by=c("province"="province_raw","date"="dates")  ) %>% 
                                  mutate( n_infected_caladj=n_infected_cal*by_city ) %>% 
                                  dplyr::select( province,city,date,n_infected_caladj ) -> df
...
generate_alphas <- function( all_dat,file_obs_cnt ) {
                ...
                cases_high_cap_loc <- cases_high_cap_loc %>% dplyr::select(Country, cases_scaled) %>% 
                ...
                for (i in 1:length(scenario_v)) {
                                ...
                                all_dat %>% filter(scenario==scenario_i) %>% 
                                                ...
                                                #
                                                dplyr::left_join( cases_high_cap_loc, by=c("destination_country"="Country") ) -> df_lin_fit
