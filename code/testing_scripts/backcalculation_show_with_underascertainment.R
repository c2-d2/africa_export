...
incidence_dat_long <- incidence_dat %>% 
  ...
  mutate(incidence = incidence*ascertainment_rate) %>%
  ## One line per person
  uncount(incidence) %>%
  ## Incubation periods
...
tmax <- 32
backwards_delays_this <- generate_gamma_distributions_backward(10, dat1, tmax)
...
for(i in 1:repeats) {
  ...
  tmp <- confirm_dat_mod_tmp %>%
    ...
    group_by(id) %>%
    mutate(
      delay = sample(seq(0, max_delay_dat-1,by=1),size=n(),
                     prob=unlist(probs)),
      delay_naive = sample(seq(0, max_delay-1,by=1), size=n(), prob=probs_overall_confirm, replace=TRUE),
  ...
  tmp_onsets <- tmp %>%
    ...
    rename(onsets=n) %>%
    mutate(i=i)
  tmp_onsets_naive <- tmp %>%
    group_by(onset_date_naive) %>%
    tally() %>%
    rename(onsets=n) %>%
    mutate(i=i)
...
combined_augmented_naive <- tmp_all_onsets_naive %>% 
  rename(onset_date=onset_date_naive) %>%
  group_by(onset_date) %>%
  summarise(lower_quant=quantile(onsets, c(0.025)),
            mid_quant1=quantile(onsets, c(0.25)),
            median=median(onsets),
...
all_augmented <- bind_rows(combined_augmented,combined_augmented_naive)
